...
我们在磁盘上保留了一个索引，使我们能够查找每个`Agents`, `Employers`和`Members`。这个索引是在磁盘上的又一个特别格式化的一组柱面。`Agents`索引由包含其ID的记录以其记录的柱面号，磁头号和扇区号组成。`Employers`和`Members`有类似的索引。`Members`也被保存在磁盘上的双向链接列表中。每个`Members`记录保存了下一个会员记录的柱面号，磁头号和扇区号，以及前一个的`Members`记录。
